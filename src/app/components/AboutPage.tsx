...
// 全部开启 float：和主页一样持续上下浮动。个人设置页也复用这份布局。
export const ABOUT_DOODLES: DoodleSpec[] = [
  { id: "cat", type: "SleepingCat", xPct: 89.1, yPct: 22.2, w: 112, opacity: 0.55, float: true },
  { id: "cloud", type: "SmilingCloud", xPct: 10, yPct: 30, w: 96, opacity: 0.5, float: true },
  { id: "mushroom", type: "Mushroom", xPct: 9.6, yPct: 51.9, w: 48, opacity: 0.45, float: true },
  { id: "wisp", type: "Wisp", xPct: 89.7, yPct: 43.5, w: 64, opacity: 0.5, float: true },
  { id: "star", type: "StarBuddy", xPct: 85.7, yPct: 68.9, w: 66, opacity: 0.55, float: true },
  { id: "crystal", type: "Crystal", xPct: 88, yPct: 85.3, w: 64, opacity: 0.5, float: true },
  { id: "teacup", type: "TeaCup", xPct: 10.9, yPct: 80.6, w: 80, opacity: 0.5, float: true },
];
...
export function AboutPage({ palette }: AboutPageProps) {
  ...
  useEffect(() => {
    api.siteContents("about.version,about.disclaimer,about.feedback").then(setContent).catch(() => {});
  }, []);

  const ver = content["about.version"]?.content_value || {};
  const appVersion: string = ver.current || APP_VERSION;
  const releaseDate: string = ver.release_date || RELEASE_DATE;
  const changelogItems: { version: string; date: string; note: string }[] =
    ver.changelog?.length ? ver.changelog : changelog;
  const disclaimerItems: string[] =
    content["about.disclaimer"]?.content_value?.items?.length ? content["about.disclaimer"].content_value.items : DEFAULT_DISCLAIMER;
  const feedback = content["about.feedback"]?.content_value || {};
  const feedbackEmail: string = feedback.email || FEEDBACK_EMAIL;
  const githubUrl: string = feedback.github_url || GITHUB_URL;
    ...
    >
      <div className="bg-flow relative min-h-full" style={{ background: palette.heroBackdrop }}>
        ...
        <div className="relative z-10 mx-auto px-6 md:px-10 py-8 max-w-2xl">
          {/* Header */}
          <Reveal immediate className="mb-8 flex flex-col items-center text-center">
            <div
            ...
            <div className="flex items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: palette.muted, fontWeight: 700 }}>更新于 {releaseDate}</div>
                </div>
              </div>
              <div
          ...
          <Card palette={palette}>
            ...
            <div className="flex flex-col gap-2.5 mt-1">
              <FeedbackRow
                ...
                label="邮件反馈"
                value={feedbackEmail}
                href={`mailto:${feedbackEmail}?subject=ToyBox 反馈`}
              />
              <FeedbackRow
                palette={palette}
          ...
          {/* Footer */}
          <Reveal className="flex items-center justify-center gap-1.5 pt-2 pb-4">
            ...
            <span style={{ fontSize: "12px", fontWeight: 700, color: palette.muted }}>慢慢搭建 · ToyBox {appVersion}</span>
          </Reveal>
        </div>
      </div>
    </div>
...
/* ---- 低饱和日系 q 版插画背景装饰：柔光色斑（关于页 / 个人设置页共用）---- */
export function AboutBlobs({ palette }: { palette: ThemePalette }) {
  return (
    <>
      <div
